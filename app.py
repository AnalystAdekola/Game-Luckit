...
def load_data():
    ...
    try:
        ...
        # CLEANING: Remove any invisible spaces from headers
        df.columns = df.columns.str.strip()
        
        # DATA TYPE FIX: Ensure 'Sum' is numeric so it matches the input
...
if st.button("RUN PEARL", type="primary"):
    if df.empty:
        st.error("Data could not be loaded. Check your Google Drive link.")
    else:
        with st.spinner("⚔️ Analyzing Old Trafford History..."):
            # We use a copy to avoid SettingWithCopy warnings
            # Ensuring we filter with normalized strings
            match1 = df[
                (df['Sum'] == sum_1) & 
            ...
            if not match1.empty or not match2.empty:
                ...
                with col_res1:
                    st.write(f"#### {st.session_state.color_1} Results")
                    if not match1.empty:
                ...
                with col_res2:
                    st.write(f"#### {st.session_state.color_2} Results")
                    if not match2.empty:
